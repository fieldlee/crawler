...
use crate::model::entitys::pagedata::PageData;
use crate::model::entitys::CommonField;
    ...
    Params: Send + Sync + Serialize,
{
    ...
    async fn list_all(&self) -> Result<Vec<Dto>> {
        let rb = APPLICATION_CONTEXT.get::<Rbatis>();
        //执行查询
        let list: Vec<Entity> = rb.fetch_list().await?;
        let result = list
            .into_iter()
            .map(|e| Dto::from(e.clone()))
            .collect::<Vec<Dto>>();
     ...
     * ytb_id
     */
    async fn get_by_ytb_id(&self, id: String) -> Result<Dto> {
        let rb = APPLICATION_CONTEXT.get::<Rbatis>();
        let wrapper = rb.new_wrapper().eq("ytb_id", id);
     ...
     * ytb_id
     */
    async fn get_by_ytb_status(&self, status: u8) -> Result<Dto> {
        let rb = APPLICATION_CONTEXT.get::<Rbatis>();
        let wrapper = rb.new_wrapper().eq("status", status);
        let detail: Entity = rb.fetch_by_wrapper(wrapper).await?;
        let vo = Dto::from(detail);
        return Ok(vo);
    }
    ...
    async fn get_by_ytb_status_times(&self, status: u8, times:u8) -> Result<Dto> {
        let rb = APPLICATION_CONTEXT.get::<Rbatis>();
        let wrapper = rb.new_wrapper().ne("status", status).lt("times", times);
        let detail: Entity = rb.fetch_by_wrapper(wrapper).await?;
        let vo = Dto::from(detail);
        return Ok(vo);
    }
