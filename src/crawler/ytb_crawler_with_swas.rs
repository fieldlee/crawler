...
/// ```
/// 
pub async fn raw_search_with_title(query: &str, page: u8) -> Result<JsonValue> {
    println!("raw_search_with_title:{:?}", format!(
        "https://youtube-scrape.herokuapp.com/api/search?q={}&page={}",
    ...
    headers.insert("User-Agent", "PostmanRuntime/7.29.2".parse().unwrap());
    headers.insert("Accept", "*/*".parse().unwrap());
    let resp = client.get(format!(
        "https://youtube-scrape.herokuapp.com/api/search?q={}&page={}",
        query, page.to_string())).headers(headers).send().await?;
...
pub async fn get_data(query: &str, page: u8) -> Result<Vec<Item>> {
    let resp = raw_search_with_title(query, page).await?;
